...
interface CyclixRandomizerInterface {
    ...
    function recoverLostNumberRequest(uint256 _requestId, uint256 numWords) external returns (uint256);
}
...
contract CyclixRandomizer is CyclixRandomizerInterface, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    struct RequestStatus {
        uint16 wordsCount; // number of words requested
    ...
    // function.
    uint32 public callbackGasLimitForOneWord = 300;
    ...
    function recoverLostNumberRequest(uint256 _requestId, uint256 numWords) public returns (uint256) {
        require(s_requests[_requestId].exists, "request not found");
