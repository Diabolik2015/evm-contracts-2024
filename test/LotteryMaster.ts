...
describe("Lottery Master", function () {
  ...
  async function deployLotteryMasterAndStartRound(timeOfRound ?: number, freeTickets ?: boolean) {
    ...
    if (timeOfRound) {
      await deployed.lotteryMaster.startNewRound((await time.latest()), (await time.latest()) + timeOfRound);
    } else {
      await deployed.lotteryMaster.startNewRound((await time.latest()), (await time.latest()) + 50);
    }
  ...
  async function addPlayersToLotteryRound(lotteryMaster: LotteryMaster) {
    await lotteryMaster.connect(player1).buyTickets( [1, 2, 3, 4, 69, 26], referral1, player1.address);
    await lotteryMaster.connect(player1).buyTickets( [1, 2, 3, 4, 69, 24], referral2, player1.address);
    await lotteryMaster.connect(player2).buyTickets( [1, 2, 3, 4, 14, 26], referral3, player2.address);
    await lotteryMaster.connect(player2).buyTickets( [1, 2, 3, 4, 14, 24], referral1, player2.address);
    await lotteryMaster.connect(player3).buyTickets( [36, 2, 3, 13, 14, 26], referral1, player3.address);
    await lotteryMaster.connect(player3).buyTickets( [1, 2, 3, 13, 14, 24], referral1, player3.address);
    await lotteryMaster.connect(referral1).buyTickets( [1, 2, 3, 4, 69, 26], hre.ethers.ZeroAddress, referral1.address);
    await lotteryMaster.connect(referral1).buyTickets( [1, 2, 3, 13, 14, 26], hre.ethers.ZeroAddress, referral1.address);
    await lotteryMaster.connect(referral2).buyTickets( [1, 2, 4, 6, 14, 24], hre.ethers.ZeroAddress, referral2.address);
    await lotteryMaster.connect(referral3).buyTickets( [1, 2, 3, 4, 14, 26], hre.ethers.ZeroAddress, referral3.address);
  }
  ...
  describe("Behaviour", function () {
    ...
    it("Should validate numbers to Join Lottery Round", async function () {
      ...
      await expect(lotteryMaster.connect(player1).buyTickets( [1, 4, 6, 10, 70, 24], hre.ethers.ZeroAddress, player1.address)).to.be.revertedWith("Invalid numbers");
      await expect(lotteryMaster.connect(player1).buyTickets( [0, 4, 6, 10, 21, 24], hre.ethers.ZeroAddress, player1.address)).to.be.revertedWith("Invalid numbers");
      await expect(lotteryMaster.connect(player1).buyTickets( [1, 4, 6, 10, 21, 27], hre.ethers.ZeroAddress, player1.address)).to.be.revertedWith("Invalid power number");
    })
    ...
    it("Should be able to Make wallets Join Lottery Round", async function () {
      ...
      await lotteryMaster.connect(player1).buyTickets( [1, 4, 6, 10, 21, 24], hre.ethers.ZeroAddress, player1.address);
      let round = await lotteryRound.getRound();
      expect(round.ticketsCount).equal(1);

      await lotteryMaster.connect(player2).buyTickets( [1, 4, 6, 10, 21, 24], player1.address, player2.address);
      await lotteryMaster.connect(player2).buyTickets( [1, 4, 6, 10, 21, 24], player1.address, player2.address);
    ...
    it("Should be able to give free rounds to a list of wallets", async function () {
      ...
      await lotteryMaster.addFreeRound([player1.address, player3.address], [1, 1])
      await lotteryMaster.connect(player1).buyTickets( [1, 4, 6, 10, 21, 24], player2.address, player1.address);
      await lotteryMaster.connect(player2).buyTickets( [1, 4, 6, 10, 21, 24], player1.address, player2.address);
      await lotteryMaster.connect(player3).buyTickets( [1, 4, 6, 10, 21, 24], player1.address, player3.address);
    ...
    it("Should rightly split winning pools", async function () {
      ...
      for (let i = 0; i < 10; i++) {
        let playerId = i % 3;
        await lotteryMaster.connect(players[playerId]).buyTickets( [1, 4, 6, 10, 21, 24], players[(i + 1) % 3], players[playerId].address);
      }
    ...
    it("Should have always different numbers for draw also if the randomizer return two times the same number", async function () {
      ...
      await lotteryMaster.closeRound((await time.latest()), (await time.latest()) + 50, 0)

      await executeChainLinkVrf(roundId, winningNumbers, winningPowerNumber, [], lotteryMaster, cyclixRandomizer, vrfMock);

      await expect(lotteryMaster.fetchRoundNumbers(roundId, (await time.latest()), (await time.latest()) + 50, 10)).to.be.fulfilled
    ...
    it("Should be able to draw public round winners", async function () {
      ...
      await expect(lotteryMaster.closeRound((await time.latest()), (await time.latest()) + 50, 1)).to.be.revertedWith("Round is not over yet")

      await time.increase(100)

      await lotteryMaster.closeRound((await time.latest()), (await time.latest()) + 50, 1)

      expect(await lotteryMaster.publicRoundRandomNumbersRequestId(roundId)).to.equal(1)
      await expect(lotteryMaster.fetchRoundNumbers(roundId, (await time.latest()), (await time.latest()) + 50, 10)).to.be.revertedWith("Random numbers not ready")
    ...
    it("Should retain the amounts of the lottery pools", async function () {
      ...
      await lotteryMaster.markWinners(roundId, (await time.latest()), (await time.latest()) + 50, [await lotteryRound.victoryTierAmounts(0) / BigInt(2), await lotteryRound.victoryTierAmounts(1),
        await lotteryRound.victoryTierAmounts(2) / BigInt(2), await lotteryRound.victoryTierAmounts(3), await lotteryRound.victoryTierAmounts(4),
        await lotteryRound.victoryTierAmounts(5) / BigInt(2), await lotteryRound.victoryTierAmounts(6)])
      const round = await lotteryRound.getRound();
      const ticketResults = []
      ...
      expect(Number(hre.ethers.formatEther(await lotteryRound.amountWonForEachTicket(0)))).to.equal(Number(hre.ethers.formatEther(await lotteryRound.victoryTierAmounts(0))) / 2)
      expect(Number(hre.ethers.formatEther(await lotteryRound.amountWonForEachTicket(1)))).to.equal(Number(hre.ethers.formatEther(await lotteryRound.victoryTierAmounts(1))))
      expect(Number(hre.ethers.formatEther(await lotteryRound.amountWonForEachTicket(2)))).to.equal(Number(hre.ethers.formatEther(await lotteryRound.victoryTierAmounts(2))) / 2)
      expect(Number(hre.ethers.formatEther(await lotteryRound.amountWonForEachTicket(3)))).to.equal(Number(hre.ethers.formatEther(await lotteryRound.victoryTierAmounts(3))))
      expect(Number(hre.ethers.formatEther(await lotteryRound.amountWonForEachTicket(4)))).to.equal(Number(hre.ethers.formatEther(await lotteryRound.victoryTierAmounts(4))))
      expect(Number(hre.ethers.formatEther(await lotteryRound.amountWonForEachTicket(5)))).to.equal(Number(hre.ethers.formatEther(await lotteryRound.victoryTierAmounts(5))) / 2)
      expect(Number(hre.ethers.formatEther(await lotteryRound.amountWonForEachTicket(6)))).to.equal(Number(hre.ethers.formatEther(await lotteryRound.victoryTierAmounts(6))))
    ...
    it("Should be able to execute more rounds bringing the pools not collected", async function () {
      ...
      await executeChainLinkVrf(roundId, winningNumbers, winningPowerNumber, referralIndexes, lotteryMaster, cyclixRandomizer, vrfMock);
      await expect(lotteryMaster.fetchRoundNumbers(roundId, (await time.latest()), (await time.latest()) + 50, 10)).to.be.fulfilled
      await lotteryMaster.markWinners(roundId, (await time.latest()), (await time.latest()) + 50, [0, 0, 0, 0, await lotteryRound.victoryTierAmounts(4), await lotteryRound.victoryTierAmounts(5) / BigInt(2), await lotteryRound.victoryTierAmounts(6)])

      expect(await lotteryRound.amountWonForEachTicket(0)).to.equal(0)
      expect(await lotteryRound.amountWonForEachTicket(1)).to.equal(0)
      expect(await lotteryRound.amountWonForEachTicket(2)).to.equal(0)
      expect(await lotteryRound.amountWonForEachTicket(3)).to.equal(0)
      expect(await lotteryRound.amountWonForEachTicket(4)).to.equal(await lotteryRound.victoryTierAmounts(4))
      expect(await lotteryRound.amountWonForEachTicket(5)).to.equal(await lotteryRound.victoryTierAmounts(5) / BigInt(2) )
      expect(await lotteryRound.amountWonForEachTicket(6)).to.equal(await lotteryRound.victoryTierAmounts(6))
      ...
      await time.increase(100)
      const roundId2 = 2
      await lotteryMaster.startNewRound((await time.latest()), (await time.latest()) + 50)
      ...
      await expect(lotteryMaster.fetchRoundNumbers(roundId2, (await time.latest()), (await time.latest()) + 50, 10)).to.be.fulfilled
      await lotteryMaster.markWinners(roundId2, (await time.latest()), (await time.latest()) + 50, [0, 0, 0, 0, 0, 0, await lotteryRound2.victoryTierAmounts(6)])
      expect(await lotteryRound2.amountWonForEachTicket(0)).to.equal(0)
      expect(await lotteryRound2.amountWonForEachTicket(1)).to.equal(0)
      expect(await lotteryRound2.amountWonForEachTicket(2)).to.equal(0)
      expect(await lotteryRound2.amountWonForEachTicket(3)).to.equal(0)
      expect(await lotteryRound2.amountWonForEachTicket(4)).to.equal(0)
      expect(await lotteryRound2.amountWonForEachTicket(5)).to.equal(0)
      expect(await lotteryRound2.amountWonForEachTicket(6)).to.equal(await lotteryRound2.victoryTierAmounts(6))
    ...
    it("Should be able to buy more tickets", async function () {
      ...
      await lotteryMaster.connect(player1).buyTickets( [1, 2, 3, 4, 69, 26, 4, 2, 1, 28, 29, 4], referral1, player1.address);
      round = await lotteryRound.getRound();
    ...
    it("Check Upgrade Works", async function() {
      ...
      await executeChainLinkVrf(roundId, winningNumbers, winningPowerNumber, referralIndexes, lotteryMaster, cyclixRandomizer, vrfMock);
      await expect(lotteryMaster.fetchRoundNumbers(roundId, (await time.latest()), (await time.latest()) + 50, 10)).to.be.fulfilled
      await lotteryMaster.markWinners(roundId, (await time.latest()), (await time.latest()) + 50, [1, 1, 1, 1, 1, 1, 1])
      await time.increase(100)

      const deployed = await deployLotteryMaster();
      await deployed.lotteryMaster.startNewRoundForUpgrade((await time.latest()), (await time.latest()) + 50, lotteryRound.getAddress(), 10);
      const lotteryRoundAddress = await deployed.lotteryMaster.rounds(Number((await deployed.lotteryMaster.roundCount())) - 1)
      const contract = await hre.ethers.getContractFactory("LotteryRound");
      // @ts-ignore
      ...
      await time.increase(100)
      await deployed.lotteryMaster.closeRound((await time.latest()), (await time.latest()) + 50, 1)
      ...
      await expect(deployed.lotteryMaster.fetchRoundNumbers(roundId, (await time.latest()), (await time.latest()) + 50, 10)).to.be.fulfilled
      await deployed.lotteryMaster.markWinners(roundId, (await time.latest()), (await time.latest()) + 50, [1, 1, 1, 1, 1, 1, 1])
      await time.increase(100)
    ...
    it("Check free rounds works well for multi purchase Works", async function() {
      ...
      await lotteryMaster.connect(player1).buyTickets( [1, 2, 3, 4, 69, 26], referral1.address, player1.address);
      expect(await lotteryMaster.freeRounds(1, player1.address)).to.be.equal(1)
      await lotteryMaster.connect(player1).buyTickets( [1, 2, 3, 4, 69, 26, 1, 2, 3, 4, 69, 26, 1, 2, 3, 4, 69, 26], referral1, player1.address);
